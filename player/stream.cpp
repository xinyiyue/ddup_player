...
Stream::Stream(stream_type_t type) : BufferConsumer(type == AUDIO_STREAM ? (char *)"audio_stream" :  (char *)"video_Stream") {
  eos_ = false;
  stream_type_ = type;
...
void *Stream::process_thread(void *arg) {
  ...
  while (!stream->dec_thread_exit_) {
    stream->consume(&data, stream->stream_type_ == AUDIO_STREAM ? AUDIO_FIFO:VIDEO_FIFO);
