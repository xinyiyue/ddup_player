...
int kiss_image_new_scaled(kiss_image *image, char *fname, kiss_array *a, int w, int h, SDL_Renderer* renderer)
{
    char buf[KISS_MAX_LENGTH];
    if (!image || !fname) return -1;
    kiss_string_copy(buf, KISS_MAX_LENGTH, RESDIR, fname);
    SDL_Surface* sourceSurface = IMG_Load(buf);
    SDL_Surface* targetSurface = SDL_CreateRGBSurface(0, w, h, 32, 0, 0, 0, 0);

    SDL_BlitScaled(sourceSurface, NULL, targetSurface, NULL);

    image->image = SDL_CreateTextureFromSurface(renderer, targetSurface);
    image->magic = KISS_MAGIC;
    return 0;
}
